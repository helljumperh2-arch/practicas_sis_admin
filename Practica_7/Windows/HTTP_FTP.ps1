...
"@
        ...
        "apache" {
            $rutaApache = $null
            $svcWmi = Get-WmiObject Win32_Service | Where-Object { $_.Name -like "Apache*" } | Select-Object -First 1
            if ($svcWmi) {
...
"@
    ...
    if (Get-NetTCPConnection -LocalPort $P -State Listen -ErrorAction SilentlyContinue) {
        ...
        Write-Host "     Acceso: http$(if ($cert.OK) {'s'})://192.168.1.235:$P" -ForegroundColor Cyan
    } else {
...
function Menu-FTP-HTTP {
    $global:FTP_IP   = (Get-NetIPAddress -AddressFamily IPv4 | Where-Object { $_.InterfaceAlias -eq "Ethernet" }).IPAddress
    ...
    while ($true) {
        ...
        Write-Host " 7) Volver al Orquestador"
        Write-Host "===================================================="
